...
from reserves_project.diagnostics import (
    KEY_VARIABLES,
    PHASE6_PREDICTORS,
    TARGET_VARIABLE,
...
def run_all_diagnostics(verbose=True, write_manifest: bool = True):
    if verbose:
        print("=" * 60)
...
def _write_manifest(results):
    config = {
        "output_dir": str(DATA_DIR / "diagnostics"),
        "n_obs": results.get("metadata", {}).get("n_obs"),
        "variables_tested": results.get("metadata", {}).get("variables_tested", []),
    }
    write_run_manifest(DATA_DIR / "diagnostics", config)
