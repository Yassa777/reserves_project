...
For emerging market economies, adequate foreign exchange reserves are the primary defence against balance-of-payments crises, currency runs, and sovereign debt distress \citep{JeanneRanciere2011}. Sri Lanka's experience between 2019 and 2022 illustrates the point starkly. Gross reserves declined from approximately US\$7.6 billion at end-2019 to an estimated US\$50 million of usable reserves by April 2022, precipitating the country's first sovereign default since independence in 1948 \citep{Athukorala2024}. The fallout was severe: GDP contracted by more than 7\%, inflation surpassed 50\%, and widespread social upheaval toppled the sitting government \citep{Weerakoon2023}.

Despite its policy relevance, reserve forecasting has attracted considerably less academic attention than adjacent macroeconomic forecasting problems. Exchange rate prediction, inflation forecasting, and GDP nowcasting each command extensive literatures \citep{Rossi2013}. However, reserve forecasting does not. Existing contributions are largely confined to single-model frameworks \citep{SehgalSharma2008,Gosselin2005} and rarely assess performance across the kind of structural breaks that make accurate forecasting most consequential. This paper addresses both shortcomings. It conducts a multi-model forecasting comparison applied to emerging market reserve dynamics, evaluated against a dataset spanning stable accumulation, pandemic disruption, sovereign default, and IMF-supervised recovery.
...
Each model class is evaluated across five variable sets of increasing dimensionality: a Parsimonious set (reserves, trade balance, exchange rate; $k=3$), a Balance of Payments set (reserves, exports, imports, remittances, tourism; $k=5$), a Monetary set (reserves, exchange rate, M2; $k=3$), a PCA-derived set (reserves plus three principal components extracted from eight indicators; $k=4$), and a Full set (all available predictors; $k=9$). Crossing multiple models with multiple information sets enables a difference-in-differences decomposition of architecture versus information effects. This distinction is consequential in small samples where structural breaks can cause richer models to overfit rather than improve.
...
Four principal findings emerge. First, Markov-Switching VAR dominates across both the parsimonious and Balance of Payments specifications, achieving RMSE of 312--315 during the post-crisis period, a 74--77\% improvement over the na\"{\i}ve benchmark. When Dynamic Model Averaging and Dynamic Model Selection are incorporated, the 90\% Model Confidence Set retains four models: MS-VAR, MS-VECM, DMA, and DMS, while eliminating all others. Pairwise Diebold-Mariano tests reinforce this boundary, with 28 of 36 comparisons significant at the 1\% level. Second, a difference-in-differences decomposition crossing model class against information set reveals that architecture dominates: MS-VAR outperforms XGBoost by approximately 464 RMSE points on average, and the interaction term confirms that model class matters substantially more than variable selection. Third, aggregating balance-of-payments flows into a scalar net flow discards approximately 94\% of the available signal through cancellation of offsetting gross flows, a loss that is most severe during crisis episodes. Fourth, the Meese-Rogoff puzzle does not hold in its strong form for reserves. MS-VAR substantially outperforms the na\"{\i}ve benchmark even during the acute crisis segment, indicating that regime-switching models can beat the random walk when applied to appropriately disaggregated data.
...
The dedicated reserve forecasting literature is limited in both volume and recency. Early contributions relied on univariate frameworks, with \cite{SehgalSharma2008} finding reasonable in-sample fit from ARIMA models applied to Indian reserves but limited out-of-sample reliability. \cite{Gosselin2005} extended the specification to a panel error-correction framework across twelve Asian and Latin American economies, documenting significant long-run relationships between reserves, imports, and short-term debt. However their model broke down in crisis episodes, precisely when accurate forecasts are most consequential.

More recent work has introduced richer model classes. \cite{Gupta2014} applied DMA and DMS to forecast China's foreign exchange reserves, finding that both methods outperform linear models and static Bayesian model averaging, with DMS the best overall performer across horizons. Their results suggest that allowing both model composition and parameters to evolve over time is more important than simply expanding the predictor set. This paper extends that finding to a regime-switching framework applied to disaggregated balance-of-payments data. A subsequent contribution by \cite{Salas2025} applied BMA-Na\"{\i}ve Bayes to reserves across 102 countries, achieving strong classification accuracy but without evaluating performance through a sovereign default episode.

Machine learning has been applied to adjacent forecasting problems with competitive results \citep{MedeirosVasconcelosVeigaZilberman2021,RichardsonMulderVehbi2021}, yet its systematic application to reserve forecasting remains limited. No existing study benchmarks classical, Bayesian, regime-switching, and machine learning approaches on identical data with formal statistical comparison, or evaluates forecast accuracy through a sovereign default.
