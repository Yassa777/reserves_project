...
\begin{abstract}
This paper compares classical econometric, Bayesian, regime-switching, and machine-learning models for forecasting Sri Lanka's foreign exchange reserves over 2005--2025, including the 2022 sovereign default episode. Using rolling-origin evaluation across five variable sets, we find that Markov-Switching VAR in a disaggregated balance-of-payments specification delivers the strongest point-forecast performance, reducing RMSE by 76.7\% relative to a random-walk benchmark in the post-default window (95\% stationary block-bootstrap CI: 67.8--83.1\%). A factorial decomposition, treated as descriptive rather than causal and paired with block-bootstrap uncertainty estimates, shows that model architecture explains more of this gain than simply expanding the information set, while the average information effect is materially smaller and less precisely estimated. These results support regime-aware, component-level reserve forecasting as a practical framework for policy surveillance in crisis-prone emerging markets.
...
The main findings are as follows. MS-VAR achieves RMSE reductions of 73.6\% (95\% CI: 63.8--79.3) and 76.7\% (67.8--83.1) relative to the na\"{\i}ve benchmark in the parsimonious and BoP specifications. On the common-support post-default window used for the 2$\times$2 comparison, the average architecture effect is 477.8 RMSE points (221.3--676.1), whereas the average information effect is 78.1 points ($-$3.1 to 149.4). Aggregating balance-of-payments flows into net flows destroys approximately 94\% of the gross flow signal. The Meese-Rogoff puzzle does not hold in its strong form: MS-VAR outperforms the random walk even during the acute crisis segment. Diagnostic analysis of regime persistence, impulse response asymmetry, and classification certainty provides analytical evidence for the sources of this advantage.
...
The merged forecasting panel spans January 2005 to December 2025 (252 monthly observations) and contains 26 candidate non-date columns drawn from the Central Bank of Sri Lanka (CBSL) statistical database, the IMF's International Reserves and Foreign Currency Liquidity (IRFCL) template, Colombo Stock Exchange market statistics, Sri Lanka Customs trade returns, and mechanically derived monthly transformations. The main-text horse race uses a theory-guided subset of seven predictors plus the reserve target, while Appendix Table~\ref{tab:predictor_screening} documents the full candidate panel, source coverage, and inclusion decisions. All monetary variables are expressed in US dollars where direct conversion is feasible. Debt and liability proxies that are only available in LKR or USD billions are rescaled for appendix robustness checks but kept outside the main-text variable sets to preserve the longer baseline estimation window. Interest-rate series exist in the broader source registry but are not harmonised into the final merged monthly evaluation panel, so the omitted-variable appendix focuses on the debt and capital-flow proxies that are actually available in the forecasting panel. Table~\ref{tab:descriptive} reports descriptive statistics for the key series used in the main specifications.
...
Appendix Table~\ref{tab:omitted_variable_robustness} adds the three omitted-variable augmentations that are feasible with the existing panel: BoP plus CSE net flows, BoP plus converted short-term debt, and BoP plus converted short-term debt plus portfolio liabilities. These horse races do not overturn the main conclusion. The winning specification remains within the regime-switching family in every case: MS-VAR leads the baseline BoP and BoP+CSE variants, while MS-VECM narrowly leads the two debt-augmented variants. Rank correlations relative to the baseline BoP ordering remain high (Spearman $\rho = 0.88$--$0.98$), so the appendix evidence is best read as a robustness check on ranking stability rather than as a case for replacing the main-text variable sets.

\subsection{Stationarity and Seasonal Properties}

All series are subjected to joint ADF and KPSS testing to determine the order of integration, and Appendix Table~\ref{tab:stationarity_outputs} reports the resulting test outputs for every usable candidate series. Gross reserves, the exchange rate, M2, imports, remittances, tourism, and the debt/liability proxies are generally non-stationary in levels, while reserve change and reserve-change percent are stationary and trade balance is trend-stationary. Seasonal properties are assessed via STL decomposition, with Appendix Table~\ref{tab:seasonality_outputs} showing seasonal strength above the code threshold of 0.3 for exports (0.455), imports (0.301), tourism (0.593), and trade balance (0.384). The baseline horse race retains the level specifications described above, while Appendix Table~\ref{tab:transformation_robustness} reports a train-sample STL seasonal-adjustment robustness check. That alternative transformation leaves the top four positions inside the regime-switching/DMA cluster and only modestly reshuffles their internal ordering.

\section{Methodology}

\subsection{Model Overview}

The model set spans classical econometrics (ARIMA, VAR, VECM), regime-switching specifications (MS-VAR, MS-VECM), Bayesian approaches (BVAR), dynamic model combination (DMA, DMS), machine learning (XGBoost, LSTM, XGB-Quantile), and na\"{\i}ve benchmarks (random walk, seasonal na\"{\i}ve). Additional specifications include a balance-of-payments identity model (BoPIdentity) and a local-level stochastic volatility model (LocalLevelSV). All models are estimated in an expanding-window fashion, initialising on the training period (January 2005--December 2019, 180 observations) and expanding monthly through the validation (2020--2022) and test (2023--2025) periods. Standard estimation details are reported in Appendix~A, with machine-learning search spaces and selected hyperparameters summarised in Tables~\ref{tab:ml_tuning_design} and~\ref{tab:ml_tuning_selected}.
...
The XGBoost implementation departs from standard applications through its rolling feature engineering pipeline, which constructs 3-month and 6-month rolling means and volatility estimates for all variables at each forecast origin. This creates a feature set that evolves with the expanding estimation window, capturing momentum and volatility clustering that are economically meaningful for reserve dynamics. The rolling construction ensures that no future information leaks into the feature set. XGB-Quantile extends the framework to produce probabilistic forecasts by predicting the 10th, 25th, 50th, 75th, and 90th percentiles of the conditional distribution. For both XGBoost and LSTM, hyperparameters are selected by three-fold expanding-window time-series cross-validation on the in-sample period, using mean validation RMSE as the selection criterion. The selected XGBoost settings are: parsimonious $(n_{\text{estimators}} = 200, \text{depth} = 3, \eta = 0.1, \text{subsample} = 0.8)$ and BoP $(n_{\text{estimators}} = 300, \text{depth} = 4, \eta = 0.1, \text{subsample} = 1.0)$. The selected LSTM settings are: parsimonious (sequence length $6$, hidden units $16$, dropout $0.2$) and BoP (sequence length $6$, hidden units $32$, dropout $0.3$), both trained for up to 60 epochs with early stopping on validation loss (patience 10). Appendix Tables~\ref{tab:ml_tuning_design} and~\ref{tab:ml_tuning_selected} report the exact candidate configurations and fold definitions.
...
Statistical significance is assessed via the \cite{DieboldMariano1995} test of equal predictive accuracy (with Harvey-Leybourne-Newbold small-sample correction) and the Model Confidence Set of \cite{HansenLundeNason2011}. For the architecture-vs-information decomposition and the headline RMSE-reduction claims, uncertainty is summarised by stationary block-bootstrap percentile intervals over rolling-origin dates. Throughout, the bootstrap uses 1,000 replications and block length $\lceil T^{1/3} \rceil$, where $T$ is the number of forecast observations. Formal definitions of all metrics are provided in Appendix~A.
...
The appendix-only predictor checks are consistent with this picture. Table~\ref{tab:omitted_variable_robustness} shows that adding CSE flows, converted short-term debt, and portfolio liabilities to the BoP set keeps the top rank inside the regime-switching family throughout. The winner shifts from MS-VAR in the baseline and CSE-augmented BoP variants to MS-VECM in the two debt-augmented variants, but XGBoost and the linear benchmarks remain materially behind. Table~\ref{tab:transformation_robustness} shows a similarly limited effect of alternative seasonal handling: train-sample STL adjustment leaves the top four positions occupied by MS-VAR, DMA, DMS, and MS-VECM in both the parsimonious and BoP families.
...
\input{../data/disentangling/table_did_rmse.tex}

Table~\ref{tab:did} presents the factorial decomposition on the common-support post-default window. The architecture effect is large and precisely estimated: averaging across variable sets, XGBoost's RMSE exceeds MS-VAR's by 477.8 points (95\% block-bootstrap CI: 221.3--676.1). The average information effect is smaller at 78.1 points, and its 95\% interval ($-$3.1 to 149.4) includes zero. Within-model contrasts show that moving from Parsimonious to BoP barely changes MS-VAR performance ($-$14.9 points; $-$54.9 to 21.6) while worsening XGBoost (171.2 points; $-$5.3 to 315.5). The interaction term is positive but imprecisely estimated (186.1 points; $-$9.4 to 345.0), so the complementarity between regime-switching architecture and BoP disaggregation should be read as suggestive rather than conclusive.
...
The central result is the strong performance of regime-aware methods. MS-VAR achieves RMSE reductions of roughly three-quarters relative to the na\"{\i}ve benchmark, with 95\% block-bootstrap intervals of 63.8--79.3\% in the parsimonious specification and 67.8--83.1\% in the BoP specification. DMA and DMS perform nearly as well, and the 90\% Model Confidence Set retains all four regime-aware specifications while eliminating all others. This ranking is stable across three alternative train/test splits. The 2$\times$2 factorial decomposition shows that the advantage is primarily architectural: the architecture effect is clearly above zero, whereas the average information effect is smaller and not distinguishable from zero at the 95\% level. The interaction term is positive but imprecisely estimated, so any complementarity claim should be treated as suggestive rather than definitive.
...
XGBoost features comprise target lags $[1, 2, 3, 6, 12]$, exogenous lags $[1, 3]$, 3- and 6-month rolling means, a 3-month rolling standard deviation, and 1- and 3-month momentum features, all shifted one period to avoid look-ahead leakage. LSTM tuning is conducted on the level panel after within-fold MinMax scaling. For both models, hyperparameters are selected by three-fold expanding-window time-series cross-validation on the training sample, with mean validation RMSE as the selection criterion. LSTM training uses early stopping on validation loss with patience 10. The code-generated search-space and selected-parameter tables are reported below.

\input{../data/model_verification/ml_tuning/table_a7_ml_tuning_design.tex}

\input{../data/model_verification/ml_tuning/table_a8_ml_tuning_selected.tex}
...
\subsection{Uncertainty for Headline Claims}

For the 2$\times$2 architecture-versus-information decomposition and the benchmark-relative RMSE reductions reported in the abstract and conclusion, uncertainty is quantified via a stationary block bootstrap over rolling-origin dates. For each bootstrap draw, dates are resampled with expected block length $\lceil T^{1/3} \rceil$, and the RMSE matrix, decomposition effects, and relative RMSE reductions are recomputed on the resampled panel. The decomposition table uses the common-support post-default window shared by all four cells (2023:01--2025:03; $n = 27$), whereas the benchmark-comparison table uses each variable set's own observed test window.

\input{../data/disentangling/table_a9_disentangling_uncertainty.tex}

\input{../data/disentangling/table_a10_headline_rmse_uncertainty.tex}

\subsection{Predictor Screening and Diagnostics}

Appendix Table~\ref{tab:predictor_screening} documents the full candidate predictor panel used in the screening exercise, including source coverage and the reason each series is retained in the main text, reserved for appendix robustness, or excluded. Appendix Tables~\ref{tab:stationarity_outputs} and~\ref{tab:seasonality_outputs} report the corresponding ADF/KPSS/Zivot-Andrews and STL/Ljung-Box outputs for every usable candidate series.

\input{../data/predictor_robustness/table_a11_predictor_screening.tex}

\input{../data/predictor_robustness/table_a12_stationarity_outputs.tex}

\input{../data/predictor_robustness/table_a13_seasonality_outputs.tex}

\subsection{Predictor and Transformation Robustness}

Appendix Table~\ref{tab:omitted_variable_robustness} reports the appendix-only horse races that add the feasible debt, liability, and capital-flow proxies already present in the merged panel. Appendix Table~\ref{tab:transformation_robustness} reports the alternative STL-based seasonal-adjustment robustness, with month-of-year factors estimated on the training sample only.

\input{../data/predictor_robustness/table_a14_omitted_variable_robustness.tex}

\input{../data/predictor_robustness/table_a15_transformation_robustness.tex}
